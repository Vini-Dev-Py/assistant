...
export async function registerKnowledgeBaseRoutes(app: FastifyInstance) {
  ...
  const knowledgeBaseController = new KnowledgeBaseController(createKnowledgeBaseEntryUseCase)

  app.post('/knowledge-base/entries', { preHandler: authMiddleware }, (request, reply) =>
    knowledgeBaseController.create(request, reply)
  )

  app.post('/test', async (request, reply) => {
    const { question } = request.body as any

    return reply.send({ message: 'Test route working', question })
  })
}
