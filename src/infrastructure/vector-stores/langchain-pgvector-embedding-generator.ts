...
export class LangchainPgvectorEmbeddingGenerator
  implements TextEmbeddingGenerator
{
  ...
  constructor(config: PgVectorConfig) {
    if (!config.connectionString) {
      throw new Error(
        "DATABASE_URL is required to initialize the PGVector embedding generator.",
      );
    }

    if (!config.openAIApiKey) {
      throw new Error(
        "An OpenAI API key is required to generate embeddings with Langchain.",
      );
    }
